...
6. Two-speed Telegram output. `PULSE` (48–69) is an explicitly non-buy breadcrumb that requires either live GMGN attention or a strong direct buyer/holder/volume inflection, plus at least two independent confirmations, clean basic safety, adequate liquidity and a precise upgrade trigger; it is capped at one per cycle. A candidate scoring 60–69 is labelled SCOUT instead whenever it also passes the stricter trade-quality gates. SCOUT/ACTION/A+ remain trade-quality calls with an uncapped evidence-led upside model, measured evidence, first-detected versus alert market cap, sellability proxy and invalidation. Standard SCOUT/ACTION/A+ floors remain 60/70/80; a narrow 55–59 SCOUT lane is allowed only for verified launchpads when independent wallet-flow checks show exceptional early acceleration and every common safety/anti-late gate passes. Target estimates are never attached to PULSE messages.
